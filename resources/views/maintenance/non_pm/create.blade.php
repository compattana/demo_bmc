...
@section('content')
    <div class="">
        <div class="arrow-steps clearfix mb-3">
            <div class="step"><span><i class="fad fa-toolbox"></i><a
                        href="{{ route('maintenance_reports.index', ['type' => $type]) }}">&nbsp;
                        ตารางการเข้าซ่อม</a></span>
            </div>
    ...
    <blockquote class="quote-info ml-0 mr-0">
        <p>
            ...
            <b>ชื่อบริษัท : </b> {{ $schedule->customer->organization_name ?? ' ' }}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        </p>
    </blockquote>

    <form
        action="{{ route('maintenance_reports.store', ['maintenance_schedule_id' => $maintenance_schedule_id, 'type' => $type]) }}"
            ...
            <div class="card-body">
                <div class="tab-content" id="custom-tabs-four-tabContent">
                    <div class="tab-pane fade show active" id="tabs-goto-report" role="tabpanel"
                        ...
                        @include('maintenance.non_pm.partials.create.technician_report')
                    </div>
                    <div class="tab-pane fade" id="tabs-goto-pm" role="tabpanel" aria-labelledby="tabs-pm">
                        @include('maintenance.non_pm.partials.create.pm')
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $.each(data, function(index, subcategory) {
                        product_serial_object.append('<option value="' + subcategory.id + '">' +
        ...
        $(document).ready(function() {
            ...
            $(".add-row-1").click(function() {
                ...
                var markup = '<tr>' + `<td>${rowCount}</td>` +
                    '<td>' +
                    ' <select class="form-control select2productPart text-center" name="product_part_present[]"><option selected value="{{ old('product_part_present[]') }}" disabled>-- กรุณาเลือก part --</option> </select>' +
                    '</td>' +
                    '<td>' +
                    '<input autocomplete="nope" type="number" class="form-control form-control-sm text-center" id="quantity_present"name="quantity_present[]" placeholder=""value="{{ old('quantity_present') }}">' +
                    '</td>' +
                    '<td>' +
        ...
        $(document).ready(function() {
            ...
            $(".add-row-2").click(function() {
                ...
                var markup = '<tr>' + `<td>${rowCount}</td>` +
                    '<td>' +
                    '<select class="form-control select2productPart text-center" name="product_part_future[]"> <option selected value="{{ old('product_part_future[]') }}" disabled>-- กรุณาเลือก part --</option> </select></td>' +
                    '<td>' +
                    '<input autocomplete="nope" type="number" class="form-control form-control-sm text-center" id="quantity_future"name="quantity_future[]" placeholder="" value="{{ old('quantity_future') }}">' +
                    '</td>' +
                    '<td>' +
                    '<button class="btn btn-danger btn-xs delete-row-1"><i class="fas fa-trash"></i></button>' +
                    '</td>' +
        ...
        $('#imageDropzone').dropzone({
            ...
            success: function(file, response) {
                ...
                $(file.previewElement).append('<input type="hidden" name="images1[]" value="' + response.name +
                    '">')
                $(file.previewElement).append('<input type="hidden" name="images1_original_name[]" value="' +
        ...
        $('#technician_id[], #customer_id').on("change", function(e) {

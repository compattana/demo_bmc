...
@section('title', 'จัดการผู้ใช้ - แก้ไขข้อมูล')

@section('content_header')
    <div class="row">
        <div class="col-sm-6">
            <h1 class="m-0 text-dark"><i class="fad nav-icon fa-fw fa-users"></i> จัดการผู้ใช้ - แก้ไขข้อมูล</h1>
        </div>
        <div class="col-sm-6">
                                ...
                                @foreach(\Spatie\Permission\Models\Role::all() as $role)
                                    @if(auth()->user()->hasRole([\App\Models\User::ROLE_SUPER_MAN]))
                                        <option value="{{$role->name}}"
                                                @if(in_array($role->name,$myrole)) selected @endif>{{$role->title}}</option>
                                    @else
